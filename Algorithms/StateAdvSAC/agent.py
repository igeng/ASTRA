...
class AdvSAC(nn.Module):
    ...
    def learn(self, step, experiences, gamma, d=1):
        ...
        actor_loss, log_pis, min_Q = self.calc_policy_loss(states, current_alpha.to(self.device))

        grad_outputs = torch.ones_like(min_Q)
        gradients = torch.autograd.grad(min_Q, states, grad_outputs=grad_outputs, create_graph=True)[0]
        gradient_penalty = self.epsilon * torch.mean(torch.norm(gradients, p=2, dim=1))
        # print("actor_loss: {}, gradients: {}, gradient_penalty: {}".format(actor_loss, gradients, gradient_penalty))
        ...
        perturbed_states = (next_states + torch.randn_like(next_states) * self.epsilon).clone().detach().requires_grad_(
            True)

        # Get predicted next-state actions and Q values from target models
        _, action_probs, log_pis = self.actor_local.evaluate(perturbed_states)
        Q_target1_next = self.critic1_target(perturbed_states)
        Q_target2_next = self.critic2_target(perturbed_states)
        min_qvalue = torch.min(Q_target1_next, Q_target2_next)
        gradients = torch.autograd.grad(min_qvalue.sum(), perturbed_states, create_graph=True)[0]
        gradient_penalty = self.epsilon * torch.norm(gradients, p=1, dim=1, keepdim=True)
        with torch.no_grad():

            Q_target_next = action_probs * (torch.min(Q_target1_next, Q_target2_next) - gradient_penalty
